...
from app.utils.contextvars import endpoint_name_contextvar, request_ip_contextvar
...
async def error_exception_handler(request: Request, exc: Exception):
    ...
    log.error(
        f"{exception_info} Server response: {content} <<< Exception: {repr(exc)}\n{traceback.format_exc()}"
    )
...
async def app_error_handler(request: Request, exc: AppHTTPException):
    return JSONResponse(
        ...
        headers=exc.headers,
        content=BaseError(
            detail=exc.detail, user_message=exc.user_message, type=exc.type
        ).model_dump(),
    )
